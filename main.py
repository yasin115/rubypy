...
# بررسی عضویت اجباری قبل از پردازش هر پیام
    ...
    👑 `راهنمای مالک` 👈 مدیریت ربات و ادمین‌های ربات
    ...
    🔹 `اصل` (ریپلای) : دیدن بیوگرافی/اصل یک کاربر
    🔹 `ربات` یا `ping` : بررسی وضعیت روشن بودن ربات
    """
    ...
    help_bot_admins = """
    👑 **مدیریت پیشرفته (مخصوص مالک و نماینده‌ها):**

    🔹 `ربات روشن` / `ربات خاموش` : تغییر وضعیت فعالیت ربات
    🔹 `ادمین کن` (ریپلای) : افزودن ادمین جدید برای ربات
    🔹 `حذف ادمین` (ریپلای) : عزل ادمین ربات
    🔹 `لیست ادمین ها` : مشاهده ادمین‌های ربات
    🔹 `ثبت مالک` (ریپلای) : تنظیم مالک اصلی گروه
    🔹 `عضویت فعال` / `عضویت غیرفعال` : کنترل جوین اجباری چنل
    🔹 `تعداد گروه‌ها` : نمایش آمار نصب ربات
    🔹 `ارسال به همه` (ریپلای) : فوروارد همگانی به تمام گروه‌ها
    🔹 `اپدیت` : بروزرسانی سورس ربات از گیت‌هاب
    """
    ...
    if text == "راهنمای قوانین":
            ...
            await update.reply(help_fun) # به راهنمای سرگرمی لینک شد
